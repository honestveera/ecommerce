...
import CreateNewCategory from "./modal";
...
const Category = () => {
  ...
  const loadCategories =(exitEditingMode)=>{
    axios
  ...
  const handleCreateNewRow = (values) => {
    if (!Object.keys(validationErrors).length) {
      ...
      axios
        .post("/api/v1/categories", params, userToken)
        .then((response) => {
          rowData["id"] = response.data.category.id;
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (!Object.keys(validationErrors).length) {
            ...
            axios
              .put(`/api/v1/categories/${row.original.id}`, params, userToken)
              .then((response) => {
                values["id"] = row.original.id;
                tableData[row.index] = values;
                setTableData([...tableData]);
                loadCategories(exitEditingMode())
                setLoader(false)
              })
              .catch((error) => console.error("Error Editing Catgeories:", error));
          }
        } else {
  ...
  const handleDeleteRow = useCallback(
    (row) => {
      ...
      axios.delete(`/api/v1/categories/${row.original.id}`, userToken).then((response) => {
